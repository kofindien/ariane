...
$logg_rsMembre = "-1";
if (isset($_GET['log'])) {
  $logg_rsMembre = $_GET['log'];
}
$clef_rsMembre = "-1";
if (isset($_GET['cle'])) {
  $clef_rsMembre = $_GET['cle'];
}
mysql_select_db($database_liaisondb, $liaisondb);
$query_rsMembre = sprintf("SELECT * FROM clients WHERE clients.email=%s AND clients.cle=%s", GetSQLValueString($logg_rsMembre, "text"),GetSQLValueString($clef_rsMembre, "text"));
$rsMembre = mysql_query($query_rsMembre, $liaisondb) or die(mysql_error());
$row_rsMembre = mysql_fetch_assoc($rsMembre);
$totalRows_rsMembre = mysql_num_rows($rsMembre);

	  	if ($row_rsMembre['etat']==1)  // Si le compte est déjà actif on prévient
          {
             $alerte= "Votre compte est déjà actif !";
          }
		else // Si ce n'est pas le cas on passe aux comparaisons
		  {
			 if($clef_rsMembre == $row_rsMembre['cle']) // On compare nos deux clés	
			   {
				  // Si elles correspondent on active le compte !	
				  $alerte= "Bienvenue <em><strong>".$row_rsMembre['nom']." ".$row_rsMembre['prenoms']."</strong></em>
				   dans l'espace de services numériques ARIANE. Votre compte a bien été activé !<br /><br />Vous pouvez donc vous connecter à votre espace client.";
										
				  // La requête qui va passer notre champ status de 0 à 1
				  $UpdateSQL = "UPDATE clients SET clients.etat = 1 WHERE clients.email='".$logg_rsMembre."'";
				  
				  mysql_select_db($database_liaisondb, $liaisondb);
				  $Result_ = mysql_query($UpdateSQL, $liaisondb) or die(mysql_error());
				  
					if ($Result_){
						$message = '<p><strong>Création et activation de compte client</strong></p>
						<p>Vous venez de recevoir la comnfirmation de création de compte d\'un nouveau client.</p>
						...
						$to = 'infos@ariane.ci';
						$objet = "Création et activation de compte d'un nouveau membre";
						mail($to, $objet, $message, $headers);
					}
			   }
			 else // Si les deux clés sont différentes on provoque une erreur...
			   {
				  $alerte = "Erreur ! Votre compte ne peut être activé...";
			   }
		  }
?>
        ...
        <div style="padding:25px; border:1px solid #CCC; min-height:380px; background:#F9F9F9 url(images/fd.png) bottom center no-repeat; border-radius:8px; -webkit-border-radius:8px; -moz-border-radius:8px;">
          <div style="line-height:1.3em;">
            ...
            <div style="margin-left:260px;">
                ...
                <div class="titre" style="margin-bottom:20px; padding:10px 0;">Activation de compte client</div>
