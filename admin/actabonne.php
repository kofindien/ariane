...
$requetes ='WHERE clients.suppr = 0 AND clients.etat = 0 AND clients.idcl = abonnements.idcl AND clients.idcl = -1';
if (isset($_GET['frmSearchAbonne']) && $_GET['frmSearchAbonne']=='abonne'){
	...
	if (isset($criteres) && count($criteres)){
		$nb = count($criteres);
		if ($nb == 1) $requetes = "WHERE clients.suppr = 0 AND clients.etat = 0 AND clients.idcl = abonnements.idcl AND ".$criteres[0];
		else {
			$requetes = "WHERE clients.suppr = 0 AND clients.idcl = abonnements.idcl AND clients.etat = 0 AND ".$criteres[0];
			for ($i = 1; $i < $nb; $i++){
...
if ((isset($_GET['id'])) && ($_GET['id'] != "")) {
   $updateSQL = sprintf("UPDATE clients SET clients.etat=1 WHERE clients.idcl=%s",
					   GetSQLValueString($_GET['id'], "int"));

  mysql_select_db($database_liaisondb, $liaisondb);
  $Result1 = mysql_query($updateSQL, $liaisondb) or die(mysql_error());

  $updateSQL = "actabonne.php?action=succes&frmSearchAbonne=abonne";
  ...
  if (isset($_GET['numcard']) && !empty($_GET['numcard'])) $updateSQL .= "&numcard=".$_GET['numcard'];
...
<div class="pull-left titre">Réactiver un abonné</div>
...
<div class="alert alert-success">
  ...
  L'abonné a été réactivé avec succès !
</div>
<?php } ?>
...
<table class="table table-striped table-hover" width="100%" border="0" cellspacing="1" cellpadding="2">
              ...
              <?php do { ?>
                ...
                wrapperClassName: 'titlebar' } )"><?php LongDateEN2dateFR($row_rsOperations['dateaj']); ?></a></td>
                    ...
                    <td><a href="actabonne.php?frmSearchAbonne=abonne&id=<?php echo $row_rsOperations['idcl']; ?><?php if (isset($_GET['nom']) && !empty($_GET['nom'])) echo '&nom=',$_GET['nom']; ?><?php if (isset($_GET['prenoms']) && !empty($_GET['prenoms'])) echo '&prenoms=',$_GET['prenoms']; ?><?php if (isset($_GET['numcard']) && !empty($_GET['numcard'])) echo '&numcard=',$_GET['numcard']; ?>" class="lien_bleu_"><i class="icon-eye-open"></i> réactiver</a></td>
                  </tr>
            ...
            <div style="margin-bottom:70px;">
                <?php
					...
					$lien = 'actabonne.php?'.$_SERVER["QUERY_STRING"];
