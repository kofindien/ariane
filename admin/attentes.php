...
$query_rsRecharge = sprintf("SELECT recharges.idr, recharges.montant, recharges.frais, abonnements.numid, clients.email FROM recharges, abonnements, clients WHERE recharges.annule = 0 AND recharges.suppr = 0 AND recharges.status = 0 AND recharges.idr = %s AND recharges.idab = abonnements.idab AND abonnements.idcl = clients.idcl", GetSQLValueString($colname_rsRecharge, "int"));
...
mysql_select_db($database_liaisondb, $liaisondb);
$query_rsTotaux = "SELECT SUM(recharges.montant) AS preleve, SUM(recharges.frais) AS fraisr, SUM(recharges.fraisp) AS plateforme, SUM(recharges.montant-recharges.frais-recharges.fraisp) AS recharge FROM recharges WHERE recharges.annule = 0 AND recharges.suppr = 0 AND recharges.status = 0";
...
mysql_select_db($database_liaisondb, $liaisondb);
$queryOperations = "SELECT * FROM recharges WHERE recharges.annule = 0 AND recharges.suppr = 0 AND recharges.status = 0";
...
mysql_select_db($database_liaisondb, $liaisondb);
$query_rsOperations = "SELECT recharges.idr, recharges.montant, recharges.frais, recharges.mpaie, recharges.cel, recharges.dateaj, recharges.status, recharges.dateserv, services.service, abonnements.numid FROM recharges, services, abonnements WHERE recharges.annule = 0 AND recharges.suppr = 0 AND recharges.status = 0 AND recharges.idab = abonnements.idab AND abonnements.ids = services.ids ORDER BY recharges.idr DESC";
...
<title>Les rechargements en attentes ::: Console d'administration</title>
<!-- InstanceEndEditable -->
...
$(document).ready(function(){
	...
	$("#Submit").click(function(){
		...
		$.post('recharger.php', {montant: montant, frais: frais, idr: idr, action: action},
		function(data){
			$("#message").html(data);
			$("#message").hide();
			$("#flash").hide();
		...
		if (motif =="" || observation =="" || action =="" || idr ==""){
		...
		$("#flash").delay(8000).fadeOut(500);
		//use the $.post() method to call insert.php file.. this is the ajax request
		$.post('recharger.php', {to: to, motif: motif, observation: observation, action: action, idr: idr},
		function(data){
			$("#message").html(data);
			$("#message").hide();
			$("#flash").hide();
...
<div class="pull-left titre">Les rechargements en attente</div>
...
<?php } ?>
    ...
    <button type="button" name="return" id="return" class="btn btn-success" onclick="window.location.replace('attentes.php');"><i class="icon-circle-arrow-left icon-white"></i> Retour</button>
    </div>
              
<?php if (isset($_GET['id']) && !empty($_GET['id']) && isset($_GET['action']) && $_GET['action']=='recharger'){ ?>
<div align="center" id="frmRecharger">
<form class="form-inline well" id="frmRecharge" name="frmRecharge">
  <label for="code">N° client ID : </label>
  <input name="code" type="text" class="input-small" id="code" value="<?php echo $row_rsRecharge['numid']; ?>" readonly="readonly">
  <label for="montant">- Montant à prélever : </label>
  <input name="montant" type="text" class="input-mini" id="montant" value="<?php echo $row_rsRecharge['montant']; ?>" readonly="readonly">
  <label for="frais">- Frais de recharge : </label>
  <input name="frais" type="text" class="input-mini" id="frais" readonly="readonly" value="<?php echo $row_rsRecharge['frais']; ?>">
  <input name="action" type="hidden" id="action" value="recharger" />
  <input name="idr" type="hidden" id="idr" value="<?php echo $row_rsRecharge['idr']; ?>" />
  <button type="submit" id="Submit" name="Submit" class="btn btn-primary"><i class="icon-white icon-share-alt"></i> Valider</button>
  <button type="button" id="button" name="button" class="btn btn-danger" onclick="window.location.replace('attentes.php');">X</button>
</form>
</div>
<?php } ?>
...
<table class="table table-bordered" cellspacing="1" cellpadding="5">
  <tr>
    ...
    <th align="center">Total montant à recharger</th>
...
<div class="btn-group">
    ...
    <ul class="dropdown-menu">
        <li>
        <a tabindex="-1" href="attentes.php?id=<?php echo $row_rsOperations['idr']; ?>&action=recharger">
        <i class="icon-share-alt"></i> Recharger</a>
        </li>
        <li><a tabindex="-1" href="attentes.php?id=<?php echo $row_rsOperations['idr']; ?>&action=annuler">
        <i class="icon-share"></i> Annuler</a></li>
    </ul>
</div>
                    </td>
                    <td><a href="recupending.php?id=<?php echo $row_rsOperations['idr']; ?>" title="Imprimer le reçu" target="_blank"><img src="../img/pdf.png" <?php RedImage("../img/pdf.png",25,0); ?> /></a></td>
                  </tr>
            ...
            <div style="margin-bottom:70px;">
                <?php
                    ...
                    echo paginate('attentes.php', '?page=', $totalPages_rsOperations, $current);
                ?>          
        </div>
<?php 
}
  ...
  Il n\'y a aucune demande de rechargement en attente...</div>';
?>
</div>
      </div>
      </div>
