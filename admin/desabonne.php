...
if ((isset($_GET['id'])) && ($_GET['id'] != "")) {
   $updateSQL = sprintf("UPDATE clients SET clients.etat=0 WHERE clients.idcl=%s",
					   GetSQLValueString($_GET['id'], "int"));

  mysql_select_db($database_liaisondb, $liaisondb);
  $Result1 = mysql_query($updateSQL, $liaisondb) or die(mysql_error());

  $updateSQL = "desabonne.php?action=succes&frmSearchAbonne=abonne";
...
<div class="pull-left titre">Désactiver un abonné</div>
...
<div class="alert alert-success">
  ...
  L'abonné a été désactivé avec succès !
</div>
<?php } ?>
...
<table class="table table-striped table-hover" width="100%" border="0" cellspacing="1" cellpadding="2">
              ...
              <?php do { ?>
                ...
                wrapperClassName: 'titlebar' } )"><?php LongDateEN2dateFR($row_rsOperations['dateaj']); ?></a></td>
                    ...
                    <td><a href="desabonne.php?frmSearchAbonne=abonne&id=<?php echo $row_rsOperations['idcl']; ?><?php if (isset($_GET['nom']) && !empty($_GET['nom'])) echo '&nom=',$_GET['nom']; ?><?php if (isset($_GET['prenoms']) && !empty($_GET['prenoms'])) echo '&prenoms=',$_GET['prenoms']; ?><?php if (isset($_GET['numcard']) && !empty($_GET['numcard'])) echo '&numcard=',$_GET['numcard']; ?>" class="lien_bleu_"><i class="icon-eye-close"></i> désactiver</a></td>
                  </tr>
            ...
            <div style="margin-bottom:70px;">
                <?php
					...
					$lien = 'desabonne.php?'.$_SERVER["QUERY_STRING"];
