...
$queryOperations = "SELECT * FROM destinations WHERE destinations.sup = 0";
...
$maxRows_rsOperations = 21; // nombre d'entrées à afficher par page (entries per page)
...
mysql_select_db($database_liaisondb, $liaisondb);
$query_rsOperations = "SELECT destinations.iddest, destinations.destination, pays.npays FROM destinations, pays WHERE destinations.idpays = pays.idpays AND destinations.sup = 0 ORDER BY destinations.destination";
...
<title>Les destinations ::: Console d'administration</title>
<!-- InstanceEndEditable -->
...
<div class="pull-left titre">Les destinations</div>
...
<tbody>
  ...
  <?php do { ?>
  <tr>
    <td width="60%"><?php echo stripslashes($row_rsOperations['destination']); ?></td>
    <td width="30%"><?php echo stripslashes($row_rsOperations['npays']); ?></td>
    <td width="10%">
    <div class="btn-group btn-small">
    <a class="btn btn-small btn-primary dropdown-toggle" data-toggle="dropdown" href="#">
     Traitements <span class="caret"></span> </a>
      <ul class="dropdown-menu">
        <li> <a tabindex="-1" href="majpays.php?id=<?php echo $row_rsOperations['idpays']; ?>">
         <i class="icon-edit"></i> Modifier</a> </li>
        <li><a tabindex="-1" href="pays.php?id=<?php echo $row_rsOperations['idpays']; ?>&action=supprimer"> 
...
<div style="margin-bottom:70px;">
              <?php
                    ...
                    echo paginate('destinations.php', '?page=', $totalPages_rsOperations, $current);
                ?>          
        </div>
      </div>
      </div>
