...
if (isset($_GET['frmUser']) && $_GET['frmUser']=='user'){
	...
	if (isset($criteres) && count($criteres)){
		$nb = count($criteres);
		if ($nb == 1) $requetes = "WHERE comptes.suppr = 0 AND comptes.active = 1 AND ".$criteres[0];
		else {
			$requetes = "WHERE comptes.suppr = 0 AND comptes.active = 1 AND ".$criteres[0];
			for ($i = 1; $i < $nb; $i++){
...
if ((isset($_GET['id'])) && ($_GET['id'] != "") && (isset($_GET['action'])) && ($_GET['action'] == "desactiver")) {
   $updateSQL = sprintf("UPDATE comptes SET comptes.active=0 WHERE comptes.idc=%s",
					   GetSQLValueString($_GET['id'], "int"));

  mysql_select_db($database_liaisondb, $liaisondb);
  $Result1 = mysql_query($updateSQL, $liaisondb) or die(mysql_error());

  $updateSQL = "desuser.php?action=succes&frmUser=user";
...
<div class="pull-left titre">Désactiver un utilisateur</div>
...
<table class="table table-striped table-hover" width="100%" border="0" cellspacing="1" cellpadding="2">
              ...
              <?php do { ?>
                  <tr>
                    ...
                    <td><a href="desuser.php?action=desactiver&id=<?php echo $row_rsUsers['idc']; ?>" class="lien_bleu_"><i class="icon-eye-close"></i> désactiver</a></td>
                  </tr>
            ...
            <div style="margin-bottom:70px;">
                <?php
                    ...
                    echo paginate('desuser.php', '?page=', $totalPages_rsUsers, $current);
                ?>          
        </div>
