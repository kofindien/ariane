...
$query_rsTotaux = "SELECT SUM(recharges.montant) AS preleve, SUM(recharges.frais) AS fraisr, SUM(recharges.fraisp) AS plateforme, SUM(recharges.montant-recharges.frais-recharges.fraisp) AS recharge FROM recharges WHERE recharges.annule = 1 AND recharges.suppr = 0 AND recharges.status = 0";
...
mysql_select_db($database_liaisondb, $liaisondb);
$queryOperations = "SELECT recharges.idr FROM recharges, abonnements, services, clients WHERE recharges.annule = 1 AND recharges.suppr = 0 AND recharges.status = 0 AND recharges.idab = abonnements.idab AND abonnements.idcl = clients.idcl AND abonnements.ids = services.ids";
...
mysql_select_db($database_liaisondb, $liaisondb);
$query_rsOperations = "SELECT recharges.idr, recharges.montant, recharges.frais, recharges.mpaie, recharges.cel, recharges.dateaj, recharges.status, recharges.dateserv, services.service, abonnements.numid FROM recharges, abonnements, services, clients WHERE recharges.annule = 1 AND recharges.suppr = 0 AND recharges.status = 0 AND recharges.idab = abonnements.idab AND abonnements.idcl = clients.idcl AND abonnements.ids = services.ids ORDER BY recharges.idr DESC";
...
<title>Les rechargements non effectués ::: Console d'administration</title>
<!-- InstanceEndEditable -->
...
<div class="pull-left titre">Les rechargements non effectués</div>
...
<table class="table table-striped table-hover" width="100%" border="0" cellspacing="1" cellpadding="2">
            ...
            <div style="margin-bottom:70px;">
                <?php
                    ...
                    echo paginate('nonrecharger.php', '?page=', $totalPages_rsOperations, $current);
                ?>          
        </div>
      </div>
      </div>
