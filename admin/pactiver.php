...
if ((isset($_POST['action'])) && ($_POST['action']=='valider') && (isset($_POST['idp'])) && !empty($_POST['idp'])){
						
	$mensualite = $_POST['mensualite'];
	$fdate =  date('Y-m-d', strtotime('+'.$mensualite.' month'));	

  $updateSQL = sprintf("UPDATE mprofil SET status = 1, ddate=%s, fdate=%s, dateserv=%s, idc=%s WHERE idp=%s",
                       GetSQLValueString(date('Y-m-d'), "date"),
                       GetSQLValueString($fdate, "date"),
  ...
  $Result1 = mysql_query($updateSQL, $liaisondb) or die(mysql_error());
	...
	if ($Result1){
		echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>L\'activation du service Mon Profil s\'est faite avec succès !</div>';
	}
	else { echo '<div class="alert alert-error"><button type="button" class="close" data-dismiss="alert">&times;</button>L\'activation du service Mon Profil a échoué... Veuillez recommencer !</div>';
	}
}

if ((isset($_POST['action'])) && ($_POST['action']=='annuler') && (isset($_POST['idp'])) && !empty($_POST['idp'])){
	
	$motif = $_POST['motif'];
	$message = $_POST['observation'];
						
  $updateSQL = sprintf("UPDATE mprofil SET annule = 1, motif = %s, observation = %s, dateserv=%s, idc=%s WHERE idp=%s",
