...
if (isset($_GET['numcard']) && !empty($_GET['numcard'])) $numcard = trim($_GET['numcard']);
else $numcard = 0;

/* Calcul du nombre total d'entrées $total dans la table documents */
mysql_select_db($database_liaisondb, $liaisondb);
 $queryOperations = "SELECT recharges.idr FROM recharges, abonnements WHERE recharges.suppr = 0 AND recharges.idab = abonnements.idab AND abonnements.numcard = '$numcard'";
...
mysql_select_db($database_liaisondb, $liaisondb);
$query_rsOperations = "SELECT recharges.idr, recharges.montant, recharges.frais, recharges.mpaie, recharges.cel, recharges.dateaj, recharges.status, recharges.dateserv, services.service, abonnements.numcard FROM recharges, services, abonnements WHERE recharges.suppr = 0 AND recharges.idab = abonnements.idab AND abonnements.ids = services.ids AND abonnements.numcard = '$numcard' ORDER BY recharges.idr DESC";
...
if ((isset($_GET['id'])) && ($_GET['id'] != "") && (isset($_GET['action'])) && ($_GET['action'] == "supprimer")) {
   $updateSQL = sprintf("UPDATE recharges SET recharges.suppr=1 WHERE recharges.idr=%s",
					   GetSQLValueString($_GET['id'], "int"));

  mysql_select_db($database_liaisondb, $liaisondb);
  $Result1 = mysql_query($updateSQL, $liaisondb) or die(mysql_error());

  $updateSQL = "suprecharge.php?action=succes&numcard=".$_GET['numcard'];
...
<div class="pull-left titre">Supprimer un rechargement</div>
...
<div align="center">
<form action="" method="get" class="form-inline well">
    ...
    <i class="icon-white icon-search"></i> Rechercher
    </button>
</form>
</div>

<?php if (isset($_GET['action']) && ($_GET['action']=='succes')){ ?>              
<div class="alert alert-success">
  <button type="button" class="close" data-dismiss="alert">&times;</button>
  La suppresion de la recharge s'est faite avec succès !
</div>
<?php } ?>
...
<table class="table table-striped table-hover" width="100%" border="0" cellspacing="1" cellpadding="2">
              ...
              <?php do { ?>
                ...
                wrapperClassName: 'titlebar' } )"><?php LongDateEN2dateFR($row_rsOperations['dateaj']); ?></a></td>
                    ...
                    <td><a href="suprecharge.php?action=supprimer&&id=<?php echo $row_rsOperations['idr']; ?>&numcard=<?php echo $_GET['numcard']; ?>" class="lien_bleu_"><i class="icon-trash"></i> supprimer</a></td>
                  </tr>
            ...
            <div style="margin-bottom:70px;">
                <?php
					...
					$lien = 'suprecharge.php?'.$_SERVER["QUERY_STRING"];
