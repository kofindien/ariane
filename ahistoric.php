...
$queryOperations = "SELECT * FROM alerts_voyages WHERE alerts_voyages.idClient = $idClient ORDER BY alerts_voyages.id DESC";
...
mysql_select_db($database_liaisondb, $liaisondb);
$query_rsOperations = "SELECT * FROM alerts_voyages WHERE  alerts_voyages.idClient = $idClient ORDER BY alerts_voyages.id DESC";
	...
	<!-- InstanceBeginEditable name="corps" -->
        ...
        <div style="padding:25px; border:1px solid #CCC; min-height:600px; background:#F9F9F9 url(images/fd.png) bottom center no-repeat; border-radius:8px; -webkit-border-radius:8px; -moz-border-radius:8px;">
          <div>
            ...
            <div style="margin-left:260px;">
                ...
                <div class="titre" style="margin-bottom:20px; padding:10px 0;">Espace client &raquo; Alertes Voyages &raquo; Historique des opérations</div>
      ...
      <div style="margin-bottom:10px;">
        ...
        <div class="pull-right" style="margin-bottom:20px;">
          Total Historique des opérations: <strong><?php echo $totalRows_rsOperations ?></strong>
          </div>
           <table data-toggle="table" class="table table-striped table-hover" width="100%" border="0" cellspacing="1" cellpadding="2" data-cache="false" data-height="299">
                  <thead>
                  <tr>
                      <th width="25%">Date</th>
                      <th>Service</th>
              ...
              <?php do { ?>
                  <tr >
                    <td width="25%">
                    <?php LongDateEN2dateFR($row_rsOperations['date_creation']); ?></td>
                    <td>Alertes Voyages </td>
                    <td><?php echo $row_rsOperations['numero_vol']; ?></td>
                    <td><?php echo $row_rsOperations['montant'] ; ?> F CFA</td>
                    <td class="e">
                        <?php
                             if($row_rsOperations['statut'] == 1){
                                 echo "<span class='label label-success'>Traitée</span>";
                             }elseif($row_rsOperations['statut'] == 2){
                        ?>
                           <span class='label label-important  '>Refusée</span><br/><br/>
                                 <a href="infosMotif.php?id=<?php echo $row_rsOperations['id']; ?>" class="lien_bleu_" onclick="return hs.htmlExpand(this, {
                objectType: 'ajax', width: '700',
                creditsPosition: 'bottom left', headingText: 'Justification concernant l\'annulation de votre démande d\'alert voyage',
                wrapperClassName: 'titlebar' } )"><span class='label label-success'>Motif</span></a>

                          <?php
                             }elseif($row_rsOperations['statut'] == 0){
                                 echo "<span class='label label-info  '>En cours...</span>";
                             }
                            ?>
                    </td>
                    <!--<td align="center"><a href="arecu.php?id=<?php echo $row_rsOperations['idav']; ?>" title="Imprimer le reçu" target="_blank"><img src="img/pdf.png" <?php //RedImage("img/pdf.png",25,0); ?> /></a></td> -->
                  </tr>
                <?php } while ($row_rsOperations = mysql_fetch_assoc($rsOperations)); ?>
              <?php
              }else{
                  ?>
                  <tr><td colspan="5"><center>Historique des opérations vide.</center></td></tr>
              <?php
                  }
              ?>
                  </tbody>
