...
mysql_select_db($database_liaisondb, $liaisondb);
$query_rsNumVols = "SELECT numvols.idnv, numvols.numero, compagnies.compagnie FROM numvols, compagnies WHERE numvols.idcp=compagnies.idcp AND compagnies.sup=0 AND numvols.sup=0 ORDER BY numvols.numero ASC";
$rsNumVols = mysql_query($query_rsNumVols, $liaisondb) or die(mysql_error());
$row_rsNumVols = mysql_fetch_assoc($rsNumVols);
$totalRows_rsNumVols = mysql_num_rows($rsNumVols);
?>
   ...
   <!-------------------- FLASH INFOS ------------------------------------->    
	...
	<!-- InstanceBeginEditable name="corps" -->
      <div style="padding:15px;">
        <div style="padding:25px; border:1px solid #CCC; min-height:500px; background:#F9F9F9 url(images/fd.png) bottom center no-repeat; border-radius:8px; -webkit-border-radius:8px; -moz-border-radius:8px;">
...
<form action="infovols.php" method="post" class="form-horizontal" id="Apply" name="Apply">
...
<div class="control-group">
  ...
  <div class="control-group">
    ...
    <div class="controls">
      <select name="vol" id="vol" required class="input-xlarge">
        <option selected disabled></option>
        <?php
		do {  
		?>
        <option value="<?php echo $row_rsNumVols['numero'], ' -- (',$row_rsNumVols['compagnie'],')'; ?>">
		<?php echo $row_rsNumVols['numero'], ' -- (',$row_rsNumVols['compagnie'],')'; ?>
        </option>
        <?php
		} while ($row_rsNumVols = mysql_fetch_assoc($rsNumVols));
		  $rows = mysql_num_rows($rsNumVols);
		  if($rows > 0) {
			  mysql_data_seek($rsNumVols, 0);
			  $row_rsNumVols = mysql_fetch_assoc($rsNumVols);
		  }
		?>
      </select>
...
jQuery(document).ready(function($){
    ...
    <div class="control-group warning">
        <label class="control-label" for="nheure">Heure du vol<span class="rouge">*</span></label>

        <div class="controls">
     <span id="sprytextfield1">
     <input name="Heurevol" type="text" required class="span1" id="heure" />
     </span>
            <span class="help-inline">H</span><span id="sprytextfield2">
     <input name="Minuitevol" type="text" required class="span1" id="minute" />
     </span>
...
<button type="submit" id="Submit" name="Submit" class="btn btn-primary"><i class="icon-search icon-white"></i> </span> Rechercher </button>
    </div>
</div>
</form>
