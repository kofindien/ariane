...
/*if ((isset($_POST['frmSend'])) && ($_POST['frmSend']=='Apply')) {
	  ...
	  $insertSQL = sprintf("INSERT INTO abonnements (idcl, ids, idd, niveau, diplome, experience, age) VALUES (%s, %s, %s, %s, %s, %s, %s)",
		   ...
		   GetSQLValueString($_POST['experience'], "int"),
		   GetSQLValueString($_POST['age'], "int"));
	
	  mysql_select_db($database_liaisondb, $liaisondb);
	  $Result1 = mysql_query($insertSQL, $liaisondb) or die(mysql_error());
  
	if ($Result1) $alerte = '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>La souscription au service Alertes Voyages a été bien prise en compte ! Vous pouvez programmer votre vol sur notre www.ariane.ci afin de recevoir les alertes.</div>';
	else $alerte =  '<div class="alert alert-error"><button type="button" class="close" data-dismiss="alert">&times;</button>La souscription au service Alertes Voyages a échoué... Veuillez recommencer !</div>';
}*/
?>
...
<div class="control-group">
  <label class="control-label" for="lservice">Service <span class="rouge">*</span></label>
    <div class="controls">
      <input name="lservice" type="text" disabled="disabled" required class="input-medium" id="lservice" value="Alertes voyages"><input name="ids" id="ids" type="hidden" value="3">
    </div>
</div>
  <div class="control-group">
    <label class="control-label" for="compagnie">Compagnie aérienne <span class="rouge">*</span></label>
    <div class="controls">
...
do {  
?>
        <option value="<?php echo $row_rsCompagny['idcp']?>"><?php echo $row_rsCompagny['compagnie']?></option>
        <?php
} while ($row_rsCompagny = mysql_fetch_assoc($rsCompagny));
  $rows = mysql_num_rows($rsCompagny);
  if($rows > 0) {
      mysql_data_seek($rsCompagny, 0);
	  $row_rsCompagny = mysql_fetch_assoc($rsCompagny);
  }
?>
      </select>
  ...
  <div class="control-group">
    <label class="control-label" for="vol">N° de vol <span class="rouge">*</span></label>
    <div class="controls">
      <input type="text" id="vol" name="vol" class="input-medium" placeholder="N° de vol" required>
    </div>
  </div>
  <div class="control-group">
    <label class="control-label" for="destination">Destination <span class="rouge">*</span></label>
    <div class="controls">
      <input type="text" id="destination" name="destination" class="input-large" placeholder="Destination" required>
    </div>
  </div>
  <div class="control-group">
    <label class="control-label" for="ddepart">Date de départ <span class="rouge">*</span></label>&nbsp;&nbsp;&nbsp;&nbsp;
    <div id="ddepart" class="input-append">
      <input type="text" id="ddepart" name="ddepart" class="input-small" placeholder="00-00-0000" required>
      <span class="add-on"><i data-time-icon="icon-time" data-date-icon="icon-calendar"></i></span>
    </div>
<script type="text/javascript">
jQuery.noConflict();
jQuery(document).ready(function($){
    $('#ddepart').datetimepicker({
		format: 'dd-MM-yyyy',
		pickTime: false
    });	
});
  ...
  <div class="control-group">
    <label class="control-label" for="hdepart">Heure de départ <span class="rouge">*</span></label>&nbsp;&nbsp;&nbsp;&nbsp;
    <div id="hdepart" class="input-append">
      <input type="text" id="hdepart" name="hdepart" class="span1" placeholder="00:00" data-format="hh:mm" required>
      <span class="add-on"><i data-time-icon="icon-time" data-date-icon="icon-calendar"></i></span>
    </div>
<script type="text/javascript">
jQuery.noConflict();
jQuery(document).ready(function($){
    $('#hdepart').datetimepicker({
	  pickDate: false,
	  pickSeconds: false,
      language: 'fr-FR'
    });	
  ...
  <div class="control-group">
    <label class="control-label" for="hlimite">Heure limite <span class="rouge">*</span></label>&nbsp;&nbsp;&nbsp;&nbsp;
    <div id="hlimite" class="input-append">
      <input type="text" id="hlimite" name="hlimite" class="span1" placeholder="00:00" data-format="hh:mm" required>
      <span class="add-on"><i data-time-icon="icon-time" data-date-icon="icon-calendar"></i></span>
    </div>
<script type="text/javascript">
jQuery.noConflict();
jQuery(document).ready(function($){
    $('#hlimite').datetimepicker({
	  pickDate: false,
	  pickSeconds: false,
      language: 'fr-FR'
    });	
