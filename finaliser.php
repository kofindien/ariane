...
if ((isset($_POST['frmSend'])) && ($_POST['frmSend']=='CreateAccount')) {
	if (!empty($_REQUEST['userCode'])){
		...
		if(md5($userCode) == $_SESSION['captcha']){
			...
			$motpasse = md5($salt2.md5($salt1.$_POST['motpass'].$salt2).$salt1);
			$mpasse = $_POST['motpass'];
			$email = $_POST['email'];
	...
	$sujet = 'Création de compte ARIANE - ESN ARIANE';
	...
	$message = "
<pre>
Bienvenue à l'ESN ARIANE ! 

Merci pour votre souscription afin d'utiliser les services de l'ESN ARIANE.

Vos paramètres d'authentification sont :
...
Ceci est un mail automatique, Merci de ne pas y répondre.
		...
		mail($email, $sujet, $message, $headers) ; // Envoi du mail	
	}
			  
			  			  			
		require_once('bin/clearsessions.php');
		}
		else {
				$_SESSION['civilite'] = $_POST['civilite'];
				$_SESSION['nom'] = $_POST['nom'];
				$_SESSION['prenoms'] = $_POST['prenoms'];
				$_SESSION['cel1'] = $_POST['cel1'];
				$_SESSION['cel2'] = $_POST['cel2'];
				$_SESSION['cel3'] = $_POST['cel3'];
				$_SESSION['code'] = $_POST['code'];
				$_SESSION['email'] = $_POST['email'];
				
				$_SESSION['alerte'] = '<h2 class="rouge">Ahem.. Recommencez !</h2>'; // Le code est erroné
				$insertGoTo = "frmcompte.php?error=captcha";
...
/* Si aucun code n'a été entré */
			...
			$_SESSION['alerte'] = '<h2 class="rouge">Etes-vous un robot ?</h2>';
			$insertGoTo = "frmcompte.php?error=nocaptcha";
			header(sprintf("Location: %s", $insertGoTo));
	}
}
?>
          ...
          <div style="line-height:1.3em;">
            ...
            <div style="margin-left:260px;">
              ...
              <div>
                ...
                <div style="padding-top:30px;">
                Merci d'avoir solliciter nos services et bienvenue dans notre espace de services numériques. <br />
                  <br />
                Veuillez vous connecter à votre espace client afin de souscrire à nos services.<br />
                <br />
                <br />
                <div class="pull-right" style="padding:10px;">
                <input name="abonnement" type="button" class="btn btn-primary" id="abonnement" value="Se connecter !" onclick="window.location.replace('login.php');" />
                </div>
                </div>
