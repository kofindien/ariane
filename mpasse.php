...
if (isset($_POST[$MM_flag])) {
  $MM_dupKeyRedirect="mpasse.php";
  
  $salt1 = "qm&h*";	$salt2 = "p#g!@";
  $usrpwd = md5($salt2.md5($salt1.$_POST['ancienpwd'].$salt2).$salt1);
  $motp = md5($salt2.md5($salt1.$_POST['motpasse'].$salt2).$salt1);
  $LoginRS__query = sprintf("SELECT motpasse FROM clients WHERE motpasse=%s", GetSQLValueString($usrpwd, "text"));
  ...
  $Result1 = mysql_query($updateSQL, $liaisondb) or die(mysql_error());
	...
	if ($Result1) $alerte = '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Votre mot de passe a été bien modifié ! Il prendra effet lors de votre prochaine connexion.</div>
	<p><button class="btn" type="button" onclick="window.location.replace(\'espaceclient.php\');">Retour</button></p>';
	else $alerte =  '<div class="alert alert-error"><button type="button" class="close" data-dismiss="alert">&times;</button>La modification de votre mot de passe a échoué... Veuillez recommencer !</div>
	<p><button class="btn" type="button" onclick="window.location.replace(\'mpasse.php\');">Retour</button></p>';
  }
  else {
	  $alerte =  '<div class="alert alert-error"><button type="button" class="close" data-dismiss="alert">&times;</button>L\'actuel mot de passe saisi est incorrect... Veuillez recommencer !</div>
	  <p><button class="btn" type="button" onclick="window.location.replace(\'mpasse.php\');">Retour</button></p>';
  }
}
...
<title>Modifier mon mot de passe ::: Ariane</title>
<!-- InstanceEndEditable -->
        ...
        <div style="padding:25px; border:1px solid #CCC; min-height:520px; background:#F9F9F9 url(images/fd.png) bottom center no-repeat; border-radius:8px; -webkit-border-radius:8px; -moz-border-radius:8px;">
          <div style="line-height:1.3em;">
            ...
            <div style="margin-left:260px;">
                ...
                <div class="titre" style="margin-bottom:20px; padding:10px 0;">Espace client &raquo; Modifier mon mot de passe</div>
...
<form action="mpasse.php" method="post" class="form-horizontal" id="frmCompte" name="frmCompte">
<div style="padding:0 0 5px 0; border-bottom:1px solid #06C; margin-bottom:15px;"><strong>Modifier le mot de passe :</strong></div>
                  <div class="row-fluid" style="margin-bottom:8px;">
                    <label class="span5" for="ancienpwd">Mot de passe actuel <span class="rouge">*</span></label>
                    <div class="span7">
                      <input type="password" name="ancienpwd" id="ancienpwd" placeholder="Mot de passe actuel" required>
                    </div>
                  </div>
                  <div class="row-fluid" style="margin-bottom:8px;">
                    <label class="span5" for="motpasse">Nouveau mot de passe <span class="rouge">*</span></label>
                    <div class="span7">
                      <input type="password" name="motpasse" id="motpasse" placeholder="Nouveau mot de passe" required>
                    </div>
                  </div>
                  <div class="row-fluid" style="margin-bottom:20px;">
                    <label class="span5" for="motpass">Répétez le nouveau mot de passe <span class="rouge">*</span></label>
                    <div class="span7"><span id="spryconfirm1">
                      <input type="password" name="motpass" id="motpass" placeholder="Répétez le nouveau mot de passe" required /><br />
