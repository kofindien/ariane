...
$query_rsProfil = sprintf("SELECT clients.idcl, clients.civilite, clients.nom, clients.prenoms, clients.cel, clients.email FROM clients WHERE clients.idcl = %s", GetSQLValueString($colname_rsProfil, "int"));
...
if ((isset($_POST['frmSend'])) && ($_POST['frmSend']=='frmCompte')) {
			
  $updateSQL = sprintf("UPDATE clients SET civilite=%s, nom=%s, prenoms=%s, email=%s, cel=%s WHERE idcl=%s",
  ...
  $Result1 = mysql_query($updateSQL, $liaisondb) or die(mysql_error());
	...
	if ($Result1) $alerte = '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Votre profil a été bien modifié ! Il prendra effet lors de votre prochaine connexion.</div>';
	else $alerte =  '<div class="alert alert-error"><button type="button" class="close" data-dismiss="alert">&times;</button>La modification de votre profil a échoué... Veuillez recommencer !</div>';
}
?>
...
<title>Modifier mon profil ::: Ariane</title>
<!-- InstanceEndEditable -->
        ...
        <div style="padding:25px; border:1px solid #CCC; min-height:520px; background:#F9F9F9 url(images/fd.png) bottom center no-repeat; border-radius:8px; -webkit-border-radius:8px; -moz-border-radius:8px;">
          <div style="line-height:1.3em;">
            ...
            <div style="margin-left:260px;">
                ...
                <div class="titre" style="margin-bottom:20px; padding:10px 0;">Espace client &raquo; Modifier mon profil</div>
...
<form action="profil.php" method="post" class="form-horizontal" id="frmCompte" name="frmCompte">
  ...
  <label class="control-label" for="civilite">Civilité <span class="rouge">*</span></label>
    <div class="controls">
        <label class="radio inline">
          <input <?php if (!(strcmp($row_rsProfil['civilite'],"Mlle"))) {echo "checked=\"checked\"";} ?> type="radio" name="civilite" id="civilite1" value="Mlle" required="required"> Mademoiselle
        </label>
        <label class="radio inline">
          <input <?php if (!(strcmp($row_rsProfil['civilite'],"Mme"))) {echo "checked=\"checked\"";} ?> type="radio" name="civilite" id="civilite2" value="Mme" required="required"> Madame
        </label>
        <label class="radio inline">
          <input <?php if (!(strcmp($row_rsProfil['civilite'],"Mr"))) {echo "checked=\"checked\"";} ?> type="radio" name="civilite" id="civilite3" value="Mr" required="required"> Monsieur
        </label>
    </div>
</div>
<div class="control-group">
  <label class="control-label" for="nom">Nom <span class="rouge">*</span></label>
    <div class="controls">
      <input name="nom" type="text" required="required" id="nom" placeholder="Nom" value="<?php echo $row_rsProfil['nom']; ?>">
    </div>
</div>
  <div class="control-group">
    <label class="control-label" for="prenoms">Prénoms <span class="rouge">*</span></label>
    <div class="controls">
      <input name="prenoms" type="text" required="required" class="input-xlarge" id="prenoms" placeholder="Prénoms" value="<?php echo $row_rsProfil['prenoms']; ?>">
    </div>
  </div>
  <div class="control-group">
    ...
    <div class="input-prepend">
      ...
      <input name="cel" type="text" required="required" class="input-small" id="cel" placeholder="00000000" value="<?php echo $row_rsProfil['cel']; ?>">
    </div>
    </div>
  </div>
  <div class="control-group">
    ...
    <div class="controls">
      <input name="email" type="email" required="required" class="input-xlarge" id="email" placeholder="email@site.com" value="<?php echo $row_rsProfil['email']; ?>">
    </div>
  </div>
  <div class="control-group">
    <div class="controls">
      <input name="idab" type="hidden" id="idab" value="<?php echo $row_rsProfil['idab']; ?>" />
      <input name="frmSend" type="hidden" id="frmSend" value="frmCompte" />
...
<button type="submit" id="Submit" name="Submit" class="btn btn-primary"><i class="icon-check icon-white"></i> Modifier</button>
    </div>
</div>
</form>
<?php } else { ?>
