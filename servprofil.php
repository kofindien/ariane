...
if ((isset($_POST['frmSend'])) && ($_POST['frmSend']=='Apply')) {
	...
	if ($Result1) $alerte = '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>La souscription au service Emplois disponible pour mon PROFIL a été bien prise en compte ! Veuillez vous acquiter des frais d\'abonnement au service pour recevoir les offres d\'emplois liées à votre profil à travers notre site www.ariane.ci</div>';
	else $alerte =  '<div class="alert alert-error"><button type="button" class="close" data-dismiss="alert">&times;</button>La souscription au service Emplois disponible pour mon PROFIL a échoué... Veuillez recommencer !</div>';
}
?>
...
<title>Souscrire au service d'emplois lié à mon PROFIL ::: Ariane</title>
<!-- InstanceEndEditable -->
        ...
        <div style="padding:25px; border:1px solid #CCC; min-height:600px; background:#F9F9F9 url(images/fd.png) bottom center no-repeat; border-radius:8px; -webkit-border-radius:8px; -moz-border-radius:8px;">
          <div style="line-height:1.3em;">
            ...
            <div style="margin-left:260px;">
                ...
                <div class="titre" style="margin-bottom:20px; padding:10px 0;">Espace client &raquo; Service emplois disponible pour mon PROFIL</div>
...
<form action="servprofil.php" method="post" class="form-horizontal" id="Apply" name="Apply">
...
<div class="control-group">
  <label class="control-label" for="lservice">Service <span class="rouge">*</span></label>
    <div class="controls">
      <input name="lservice" type="text" disabled="disabled" required class="input-xlarge" id="lservice" value="Emplois disponible pour mon PROFIL"><input name="ids" id="ids" type="hidden" value="2">
    </div>
</div>
  <div class="control-group">
    ...
    <div class="controls" style="padding-top:13px;">
      <select id="idd" name="idd" class="input-large" required>
		<?php do { ?>
            <option value="<?php echo $row_rsDomaines['idd']?>"><?php echo $row_rsDomaines['domaine']?></option>
        <?php
  ...
  <div class="control-group">
    ...
    <div class="controls">
        <select name="niveau" id="niveau" class="span5" required>
          ...
          <option value="CEPE">CEPE (CAP)</option>
          <option value="BEPC">BEPC (BEP/BEPC)</option>
          <option value="Tle">Niveau Terminale</option>
          <option value="BAC">BAC (BT/BAC)</option>
          <option value="BAC+2">BAC+2 (DUT/DEUG/DUEL/BTS)</option>
          <option value="BAC+3">BAC+3 (Licence)</option>
          <option value="BAC+4">BAC+4 (Maîtrise/Master 1)</option>
          <option value="BAC+5">BAC+5 (DEA/DESS/MBA/Master 2/Ingénieur/Architecte)</option>
          <option value="BAC+6">BAC+6 (Doctorat du 3ème cycle)</option>
          <option value="BAC+7">BAC+7 (Doctorat d'Etat)</option>
        </select>
    </div>
  </div>
  <div class="control-group">
    ...
    <div class="controls">
      <input type="text" id="diplome" name="diplome" class="span5" required>
    </div>
  </div>
  <div class="control-group">
    <label class="control-label" for="experience">Nombre d'années d'expériences <span class="rouge">*</span></label>
    <div class="controls" style="padding-top:13px;">
      <input type="text" id="experience" name="experience" class="span1" placeholder="0" required> an(s)
    </div>
  </div>
  <div class="control-group">
    <label class="control-label" for="age">Age <span class="rouge">*</span></label>
    <div class="controls">
      <input type="text" id="age" name="age" class="span1" placeholder="0" required>
    ans</div>
  </div>
  <div class="control-group">
    <div class="controls">
