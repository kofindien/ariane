...
jQuery(document).ready(function($){
	...
	$("#Submit").click(function(){
		...
		var ctransact=$("#ctransact").val();
		var frais=$("#frais").val();
		var fom=$("#fom").val();
		var idab=$("#idab").val();
		
		if (mpaie =="" || montant =="" || ctransact =="" || frais =="" || fom ==""){
		...
		$.post('insertrecharge.php', {mpaie: mpaie, montant: montant, ctransact: ctransact, frais: frais, idab: idab, fom: fom},
		function(data){
			$("#message").html(data);
			$("#message").hide();
			$("#flash").hide();
...
<script type="text/javascript">	
	/*function mPay(){
		if (document.getElementById('mpaie').options[document.getElementById('mpaie').selectedIndex].text!="Orange Money"){
	...
	function cFrais(){
		...
		if (document.getElementById('mpaie').options[document.getElementById('mpaie').selectedIndex].text=="Orange Money"){
		if ((mt >= 200) && (mt <= 5000)) fom = 150;
		if ((mt >= 5005) && (mt <= 25000)) fom = 500;
		if ((mt >= 25005) && (mt <= 50000)) fom = 1000;
		if ((mt >= 50005) && (mt <= 100000)) fom = 2000;
		if ((mt >= 100005) && (mt <= 250000)) fom = 4000;
		if ((mt >= 250005) && (mt <= 500000)) fom = 8000;
		if ((mt >= 500005) && (mt <= 1000000)) fom = 16000;
		if ((mt >= 1000005) && (mt <= 1500000)) fom = 24000;

			if (mt <= 55000) frais =1100;
			else frais = Math.round(parseFloat(((mt-fom)*0.02)+100));
			mtr = Math.round(mt-fom-frais);
			document.forms["frmRecharge"].fom.value = fom;
			document.forms["frmRecharge"].mtr.value = mtr;
			document.forms["frmRecharge"].frais.value = frais;
		}
		if (document.getElementById('mpaie').options[document.getElementById('mpaie').selectedIndex].text=="MTN Mobile Money"){
		if ((mt >= 5) && (mt <= 5000)) fom = 150;
		if ((mt >= 5005) && (mt <= 25000)) fom = 500;
		if ((mt >= 25005) && (mt <= 50000)) fom = 1000;
		if ((mt >= 50005) && (mt <= 100000)) fom = 1700;
		if ((mt >= 100005) && (mt <= 200000)) fom = 3500;
		if ((mt >= 200005) && (mt <= 300000)) fom = 6000;
		if ((mt >= 300005) && (mt <= 500000)) fom = 7000;
		if ((mt >= 500005) && (mt <= 1000000)) fom = 10000;
		if ((mt >= 1000005) && (mt <= 1500000)) fom = 15000;
		...
		if (document.getElementById('mpaie').options[document.getElementById('mpaie').selectedIndex].text=="Flooz"){
			if ((mt >= 5) && (mt <= 5000)) fom = 250;
			if ((mt >= 5001) && (mt <= 25000)) fom = 500;
			if ((mt >= 25001) && (mt <= 50000)) fom = 800;
			if ((mt >= 50001) && (mt <= 100000)) fom = 1500;
			if ((mt >= 100001) && (mt <= 200000)) fom = 3000;
			if ((mt >= 200001) && (mt <= 300000)) fom = 4700;
			if ((mt >= 300001) && (mt <= 500000)) fom = 6000;
...
<div class="control-group">
  <label class="control-label" for="mpaie">Moyen de paiement <span class="rouge">*</span></label>
    <div class="controls">
    <select name="mpaie" class="input-large" id="mpaie">
    	<option selected="selected"></option>
        <option value="Orange Money">Orange Money</option>
        <option value="MTN Mobile Money">MTN Mobile Money</option>
        <option value="Flooz">Flooz</option>
        <option value="Bankcell" disabled="disabled">Bankcell</option>
    </select>
    </div>
</div>
  <div class="control-group">
    <label class="control-label" for="montant">Montant transféré <span class="rouge">*</span></label>
    <div class="controls">
    <div class="input-append">
      <input type="text" id="montant" name="montant" class="input-small" placeholder="0" required onblur="cFrais();" onchange="cFrais();" onfocus="cFrais();">
      <span class="add-on">F CFA</span>
    </div>
    &nbsp;&nbsp;&nbsp;hors frais de transfert</div>
  </div>
  <div id="idfom" class="control-group">
    <label class="control-label" for="fom">Frais de retrait</label>
    <div class="controls">
    <div class="input-append">
      <input type="text" id="fom" name="fom" class="input-small" placeholder="0" readonly="readonly" disabled="disabled">
      <span class="add-on">F CFA</span>
    </div>
    </div>
  </div>
  <div class="control-group">
    <label class="control-label" for="frais">Frais de recharge</label>
    <div class="controls">
    <div class="input-append">
        <input type="text" id="frais" name="frais" class="input-small" placeholder="0" readonly="readonly">
      <span class="add-on">F CFA</span>
    </div>&nbsp;&nbsp;**
    </div>
    <div class="alert" style="margin-top:15px;">
        <em><strong>**</strong> : Les montants liés au frais de rechargement seront arrondis au franc supérieur au cours du rechargement. Ex: 1233 f CFA sera 1235 f CFA</em><br /><br />
NB: 1 000 f CFA si montant inférieur ou égale 50 000 f CFA et 2% au délà.</div>
  </div>
  <div class="control-group">
    <label class="control-label" for="mtr">Montant à recharger</label>
    <div class="controls">
    <div class="input-append">
      <input type="text" id="mtr" name="mtr" class="input-small" placeholder="0" readonly="readonly" style="font-weight:bold; color:#F00;">
      <span class="add-on">F CFA</span>
    </div>
    </div>
  </div>
