...
mysql_select_db($database_liaisondb, $liaisondb);
$query_rsAbonnement = sprintf("SELECT * FROM abonnements WHERE abonnements.idab = %s AND abonnements.idcl = %s", GetSQLValueString($colname_rsAbonnement, "int"), GetSQLValueString($_SESSION['ariane_user_id'], "int"));
...
if ((isset($_POST['frmSend'])) && ($_POST['frmSend']=='Apply')) {
	  ...
	  $updateSQL = sprintf("UPDATE abonnements SET numcard = %s, dvalidite = %s, numid = %s  WHERE idab=%s",
		   GetSQLValueString($_POST['numcard'], "text"),
		   GetSQLValueString($_POST['dvalidite'], "text"),
		   GetSQLValueString($_POST['numid'], "text"),
		   GetSQLValueString($_POST['idab'], "int"));
	
	  mysql_select_db($database_liaisondb, $liaisondb);
	  $Result1 = mysql_query($updateSQL, $liaisondb) or die(mysql_error());
  
	if ($Result1) $alerte = '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>La modification des informations de la souscription au service de rechargement de carte prépayée VISA UBA Africard a été bien prise en compte !</div>';
...
<title>Souscrire au service de rechargement de carte prépayée VISA UBA Africard ::: Ariane</title>
<!-- InstanceEndEditable -->
   ...
   <!-------------------- FLASH INFOS ------------------------------------->    
	...
	<!-- InstanceBeginEditable name="corps" -->
      <div style="padding:15px;">
        <div style="padding:25px; border:1px solid #CCC; min-height:470px; background:#F9F9F9 url(images/fd.png) bottom center no-repeat; border-radius:8px; -webkit-border-radius:8px; -moz-border-radius:8px;">
          <div style="line-height:1.3em;">
            ...
            <div style="margin-left:260px;">
                ...
                <div class="titre" style="margin-bottom:20px; padding:10px 0;">Espace client &raquo; Service de carte prépayée VISA UBA Africard</div>
...
<form action="upservafricard.php?id=<?php echo $_GET['serv']; ?>" method="post" class="form-horizontal" id="Apply" name="Apply">
...
<div class="control-group">
  <label class="control-label" for="lservice">Service <span class="rouge">*</span></label>
    <div class="controls">
      <input name="lservice" type="text" disabled="disabled" required class="span4" id="lservice" value="Rechargements de cartes VISA UBA Africard"><input name="ids" id="ids" type="hidden" value="1">
    </div>
</div>
  <div class="control-group">
    <label class="control-label" for="numcard">N° de la carte <span class="rouge">*</span></label>
    <div class="controls">
      <input type="text" id="numcard" name="numcard" class="input-medium" required value="<?php echo $row_rsAbonnement['numcard']; ?>">
    </div>
  </div>
  <div class="control-group">
    <label class="control-label" for="dvalidite">Date de validité <span class="rouge">*</span></label>
    <div class="controls">
    <input type="text" id="dvalidite" name="dvalidite" class="input-mini" required maxlength="5" value="<?php echo $row_rsAbonnement['dvalidite']; ?>">&nbsp;&nbsp;(mm/aa; ex. : 10/16)
    </div>
  </div>
  <div class="control-group">
    <label class="control-label" for="numid">N° Client ID <span class="rouge">*</span></label>
    <div class="controls">
      <input name="numid" type="text" required class="input-small" id="numid" value="<?php echo $row_rsAbonnement['numid']; ?>" maxlength="10">&nbsp;&nbsp;&nbsp;(<a href="images/versoafricard.jpg" class="highslide" onclick="return hs.expand(this)">Voir où trouver le n° sur la carte</a>) </div>
  </div>
  <div class="control-group">
    <div class="controls">
